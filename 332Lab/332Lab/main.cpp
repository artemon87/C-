...
bool sortInput(istream &infile, StudentType students [], int &size)
{
   // int circle=0;
    ...
    StudentType tempStruct; //constracts/initializes temprorary StudentType
 //   char error[MAXLENGTH];
    ...
    while(size<MAXSIZE &&!infile.eof())
    {
        ...
        size++;
        strcpy(tempStruct.last, students[size-1].last);
        strcpy(tempStruct.first, students[size-1].first);
        tempStruct.grade=students[size-1].grade;
        // circle++; //makes circle to be 1;
        //infile >> temp;
        // }
        // else if(strcmp(temp, "error") != 0 && circle!=0)
        //{
        //  strcpy(tempStruct.last, temp);
        //  infile >> tempStruct.first >> tempStruct.grade;
        ...
        for(i=size-1; i>0; i--)
        {
            ...
            else if(strcmp(tempStruct.last, students[i-1].last)<0 && size>1) //comares l.names
            {
                //strcpy(tempStruct.last, students[size-1].last);
                //strcpy(tempStruct.first, students[size-1].first);
                //tempStruct.grade=students[size-1].grade;
            ...
            {
                //strcpy(students[i].last, tempStruct.last);  //copies temprorary
                //strcpy(students[i].first, tempStruct.first);//stored studen's
                //students[i].grade=tempStruct.grade;         //info into correct
                //sorted position
                //size++;
                //infile>>temp;
